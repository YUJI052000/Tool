...
class create:
    ...
    'authority': 'm.facebook.com',
        ...
        for x in range(lim):
            self.loop += 1
            sys.stdout.write(f'\r {OO}[YUJI-CREATING] {OO}{self.loop}/{str(lim)} | ALIVE: {len(ok)} | CHECKPOINT: {len(cp)}{OO} '),
            sys.stdout.flush()
                        ...
                        for x in BeautifulSoup(po.text, 'html.parser').find_all('form', {'method': 'post'}):
                            if 'confirmation_event_location=cliff' in str(x):
                                ...
                                if 'checkpoint' in rex.url:
                                    ...
                                    cok = self.ses.cookies.get_dict()
                                    print ('\r\033[1;32m [YUJI-ALIVE] '+cok['c_user']+' | '+passw+' | '+coki+'\033[0;97m     ')
